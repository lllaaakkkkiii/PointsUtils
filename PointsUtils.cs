...
namespace PointUtils
{
    public class PointUtils : Plugin
    {
        ...
        {
            base.OnPluginInit();
            Debug.Log("\u001b[32m[By Spicy...ice] Points Utils ON\u001b[0m");
        ...
        {
            UIPanel garagePanel = new UIPanel("garagePanel", UIPanel.PanelType.TabPrice);
            ...
            foreach (var v in Nova.v.vehicles)
            {
                ...
                Item item = Nova.man.item.GetItem(v.vehicleId);

                garagePanel.AddTabLine($"Plaque: {v.plate}", $"{stowed}", Nova.man.newIcons.IndexOf(item.Icon), _ =>
                {
                    if (v.isStowed)
                    {
                    ...
                    else { player.Notify("<color=#D6C53E>! Warning !</color>", "Le véhicule est déjà sorti !", NotificationManager.Type.Warning, 5); return; }
                });
            }
